...
class CefAllowCertificateErrorCallbackImpl : public CefRequestCallback {
 ...
 private:
  static void RunNow(CallbackType callback, bool allow) {
    CEF_REQUIRE_UIT();
    std::move(callback).Run(allow ? content::CERTIFICATE_REQUEST_RESULT_TYPE_CONTINUE
...
CefContentBrowserClient::CreateClientCertStore(
  ...
  if (!browser_context)
    return nullptr;
  return static_cast<CefResourceContext*>(static_cast<CefBrowserContext*>(browser_context)
      ->GetResourceContext())

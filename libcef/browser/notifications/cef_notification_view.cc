...
#include "libcef/browser/notifications/cef_notification_view.h"
...
using namespace message_center;
...
static SkColor kActionsRowBackgroundColor = SkColorSetRGB(0x26, 0x26, 0x26);
...
static SkColor kLargeImageBackgroundColor = SkColorSetRGB(0x33, 0x39, 0x47);
static SkColor kCImageBackgroundColor = SkColorSetRGB(0x33, 0x39, 0x47);

static SkColor kRegularTextColorMD = SkColorSetRGB(0xE2, 0xE2, 0xE2);
static SkColor kDimTextColorMD = SkColorSetRGB(0xC0, 0xC0, 0xC0);

static SkColor kCNotificationBackgroundColor = SkColorSetRGB(0x33, 0x39, 0x47);
...
constexpr int kCefCompactTitleMessageViewSpacing = 12;
...
constexpr int kLineHeightMD = 20;
...
class ClickActivator : public ui::EventHandler {
 public:
  explicit ClickActivator(CefNotificationView* owner) : owner_(owner) {}
 ...
 private:
  ...
  CefNotificationView* const owner_;
...
// CefCompactTitleMessageView
// /////////////////////////////////////////////////////

CefCompactTitleMessageView::~CefCompactTitleMessageView() = default;

const char* CefCompactTitleMessageView::GetClassName() const {
  return "CefCompactTitleMessageView";
}

CefCompactTitleMessageView::CefCompactTitleMessageView() {
  const gfx::FontList& font_list = GetTextFontList();
...
void CefCompactTitleMessageView::Layout() {
  ...
  //   kCefCompactTitleMessageViewSpacing is added between them, and the elided
  //   title is shown.
  ...
  const int title_width =
      std::max(0, width() - message_width - kCefCompactTitleMessageViewSpacing);

  title_->SetBounds(0, 0, title_width, height());
  message_->SetBounds(width() - message_width, 0, message_width, height());
}

void CefCompactTitleMessageView::set_title(const base::string16& title) {
  title_->SetText(title);
}

void CefCompactTitleMessageView::set_message(const base::string16& message) {
...
void CefNotificationView::CreateOrUpdateViews(
  ...
  CreateOrUpdateCefCompactTitleMessageView(notification);
...
CefNotificationView::CefNotificationView(const Notification& notification)
  ...
  auto color_scheme = native_theme->GetPreferredColorScheme();
  if (color_scheme == ui::NativeTheme::PreferredColorScheme::kDark) {
    kActionsRowBackgroundColor = SkColorSetRGB(0x26, 0x26, 0x26);
    kLargeImageBackgroundColor = SkColorSetRGB(0x33, 0x39, 0x47);
    kCImageBackgroundColor = SkColorSetRGB(0x33, 0x39, 0x47);

    kRegularTextColorMD = SkColorSetRGB(0xE2, 0xE2, 0xE2);
    kDimTextColorMD = SkColorSetRGB(0xC0, 0xC0, 0xC0);
    kCNotificationBackgroundColor = SkColorSetRGB(0x33, 0x39, 0x47);
  } else {
    // Set light colors
    kActionsRowBackgroundColor = SkColorSetRGB(0xbc, 0xcb, 0xeb);
    kLargeImageBackgroundColor = SkColorSetRGB(0xf5, 0xf5, 0xf5);
    kCImageBackgroundColor = SkColorSetRGB(0xf5, 0xf5, 0xf5);

    kRegularTextColorMD = SkColorSetRGB(0x1a, 0x1a, 0x1a);
    kDimTextColorMD = SkColorSetRGB(0x63, 0x63, 0x63);
    kCNotificationBackgroundColor = SkColorSetRGB(0xf5, 0xf5, 0xf5);
  }
  ...
  // - CefNotificationView::OnMousePresssed would not fire on the inline reply
  ...
  SetBorder(views::CreateSolidBorder(2, SkColorSetRGB(0x25, 0x80, 0xC3)));
}

CefNotificationView::~CefNotificationView() {
  RemovePreTargetHandler(click_activator_.get());
}

void CefNotificationView::Layout() {
  MessageView::Layout();
...
void CefNotificationView::OnFocus() {
  MessageView::OnFocus();
  ScrollRectToVisible(GetLocalBounds());
}

bool CefNotificationView::OnMousePressed(const ui::MouseEvent& event) {
  last_mouse_pressed_timestamp_ = base::TimeTicks(event.time_stamp());
  return true;
}

bool CefNotificationView::OnMouseDragged(const ui::MouseEvent& event) {
  return true;
}

void CefNotificationView::OnMouseReleased(const ui::MouseEvent& event) {
...
void CefNotificationView::OnMouseEvent(ui::MouseEvent* event) {
  switch (event->type()) {
...
void CefNotificationView::OnGestureEvent(ui::GestureEvent* event) {
...
void CefNotificationView::UpdateWithNotification(
    const Notification& notification) {
...
void CefNotificationView::UpdateControlButtonsVisibilityWithNotification(
    const Notification& notification) {
  control_buttons_view_->ShowSettingsButton(false);
  control_buttons_view_->ShowSnoozeButton(false);
  control_buttons_view_->ShowCloseButton(GetMode() != Mode::PINNED);
  UpdateControlButtonsVisibility();
}

void CefNotificationView::ButtonPressed(views::Button* sender,
                                        const ui::Event& event) {
...
void CefNotificationView::OnNotificationInputSubmit(
    size_t index,
...
void CefNotificationView::CreateOrUpdateContextTitleView(
    const Notification& notification) {
...
void CefNotificationView::CreateOrUpdateTitleView(
    const Notification& notification) {
...
void CefNotificationView::CreateOrUpdateMessageView(
    const Notification& notification) {
...
void CefNotificationView::CreateOrUpdateCefCompactTitleMessageView(
    const Notification& notification) {
  if (notification.type() != NOTIFICATION_TYPE_PROGRESS) {
  ...
  if (!compact_title_message_view_) {
    compact_title_message_view_ = new CefCompactTitleMessageView();
...
void CefNotificationView::CreateOrUpdateProgressBarView(
    const Notification& notification) {
  if (notification.type() != NOTIFICATION_TYPE_PROGRESS) {
...
void CefNotificationView::CreateOrUpdateProgressStatusView(
    const Notification& notification) {
  if (notification.type() != NOTIFICATION_TYPE_PROGRESS ||
...
void CefNotificationView::CreateOrUpdateListItemViews(
    const Notification& notification) {
...
void CefNotificationView::CreateOrUpdateIconView(
    const Notification& notification) {
...
void CefNotificationView::CreateOrUpdateSmallIconView(
    const Notification& notification) {
...
void CefNotificationView::CreateOrUpdateImageView(
    const Notification& notification) {
  ...
  if (!image_container_view_) {
    ...
    image_container_view_->SetBackground(
        views::CreateSolidBackground(kCImageBackgroundColor));
...
void CefNotificationView::CreateOrUpdateActionButtonViews(
    const Notification& notification) {
...
void CefNotificationView::CreateOrUpdateInlineSettingsViews(
    const Notification& notification) {
  if (settings_row_) {
...
bool CefNotificationView::IsExpandable() {
...
void CefNotificationView::ToggleExpanded() {
  SetExpanded(!expanded_);
}

void CefNotificationView::UpdateViewForExpandedState(bool expanded) {
...
void CefNotificationView::ToggleInlineSettings(const ui::Event& event) {
  if (!settings_row_)
    return;
...
void CefNotificationView::SetDrawBackgroundAsActive(bool active) {
  background()->SetNativeControlColor(active ? kHoveredButtonBackgroundColor
                                             : SkColorSetRGB(0x33, 0x39, 0x47));
  SchedulePaint();
}

void CefNotificationView::UpdateCornerRadius(int top_radius,
                                             int bottom_radius) {
  SetCornerRadius(0, 0);
  SetBackground(views::CreateBackgroundFromPainter(
      std::make_unique<NotificationBackgroundPainter>(
          top_radius, bottom_radius, kCNotificationBackgroundColor)));
...
NotificationControlButtonsView* CefNotificationView::GetControlButtonsView()
    const {
  return control_buttons_view_.get();
}

bool CefNotificationView::IsExpanded() const {
  return expanded_;
}

void CefNotificationView::SetExpanded(bool expanded) {
...
bool CefNotificationView::IsManuallyExpandedOrCollapsed() const {
  return manually_expanded_or_collapsed_;
}

void CefNotificationView::SetManuallyExpandedOrCollapsed(bool value) {
  manually_expanded_or_collapsed_ = value;
}

void CefNotificationView::OnSettingsButtonPressed(const ui::Event& event) {
  if (settings_row_)
    ToggleInlineSettings(event);
  else
...
void CefNotificationView::Activate() {
  GetWidget()->widget_delegate()->SetCanActivate(true);
  GetWidget()->Activate();
}

void CefNotificationView::AddBackgroundAnimation(const ui::Event& event) {
  ...
  // |control_buttons_view_| to that of CefNotificationView, create a new
...
void CefNotificationView::RemoveBackgroundAnimation() {
  SetInkDropMode(InkDropMode::OFF);
  AnimateInkDrop(views::InkDropState::HIDDEN, nullptr);
}

void CefNotificationView::AddInkDropLayer(ui::Layer* ink_drop_layer) {
...
void CefNotificationView::RemoveInkDropLayer(ui::Layer* ink_drop_layer) {
...
std::unique_ptr<views::InkDrop> CefNotificationView::CreateInkDrop() {
  return std::make_unique<NotificationInkDropImpl>(this, size());
}

std::unique_ptr<views::InkDropRipple> CefNotificationView::CreateInkDropRipple()
    const {
  return std::make_unique<views::FloodFillInkDropRipple>(
...
void CefNotificationView::InstallNotificationInkDropMask() {
  SkPath path;
...
std::unique_ptr<views::InkDropMask> CefNotificationView::CreateInkDropMask()
    const {
...
SkColor CefNotificationView::GetInkDropBaseColor() const {
  // Background of inline settings area.
  return SkColorSetRGB(0xEE, 0xEE, 0xEE);
}

void CefNotificationView::InkDropAnimationStarted() {
  header_row_->SetSubpixelRenderingEnabled(false);
}

void CefNotificationView::InkDropRippleAnimationEnded(
